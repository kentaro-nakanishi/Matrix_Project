...
class Path:
    ...
    def to_bool(self, bool_pattern=0, dim_pattern={}):
        ...
        if bool_pattern == 0:##org_treeにて、縮約番号の指示を出す
            ...
            return ((D-U+R)<=4)  #深度の偶奇で縮約、非縮約を分ける
...
class Node:
    ...
    def do_contraction(self,  dim_pattern={}):
        ...
        if len(self.next) == 2:
            ...
            copy_left_tree.make_brother(copy_right_tree,dim_pattern=dim_pattern)
...
class SubNode(Node):
    ...
    def make_brother(self,copy_right_tree, path = Path(adress=[]),   dim_pattern={}):
        ...
        if True:#self.mother.root:#root上では循環-変換代数が実装できない(→A方式ならできるはず)
            ...
            joint_node = self.mother.root#A方式
